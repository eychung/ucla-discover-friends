...
public class DataReceiverService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		if (intent.getAction().equals(Constants.NETWORK_INITIATOR_GET_CERTIFICATE)) {
			byte[] ecf = intent.getByteArrayExtra(Constants.EXTRAS_ENCRYPTED_CERTIFICATE);
			new InitiatorGetCertificateTask().execute(ecf);
		}
			...
			new InitiatorMessageListenerTask().execute(encryptedMessage);
		}
			...
			new TargetMessageListenerTask().execute(publicKey);
		}
	}
